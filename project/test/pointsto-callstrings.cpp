...
{
	return (TREE_CODE (SSAVAR (csvarmap[varid]->decl)) 
			== PARM_DECL);
}
...
{
	return csvarmap[varid]->scoping_function;
}
...
{
	return (csvarmap[varid]->is_global_var);
}
...
{
	callstring ai =  callstrings.index ( cs);
	return ai->bb;
}
...
print_pointsto_list (int index)
{
	...
	if(index < 0)
		...
		EXECUTE_IF_SET_IN_BITMAP(lhs_bit,0,var,bir)
		{
			...
			*/	bitmap pointee = get_pointee_set(index,maylist,var);
			...
			fprintf (dump_file, "  %s", csvarmap[var]->name);
			fprintf (dump_file, "->{");
			EXECUTE_IF_SET_IN_BITMAP(pointee, 0, rhsbit, bi)
			{
				fprintf (dump_file, "%s", csvarmap[rhsbit]->name);
				if(i++ < cnt)
					fprintf (dump_file, ",");
			}
			if(!cnt){
	...
	/*   if (info == NULL)
		 ...
		 fprintf (dump_file, "  %s", csvarmap[ai->lhs]->name);
		 fprintf (dump_file, "->{");
...
{
	unsigned index = csvarmap.length ();
...
cs_get_vi_for_tree (tree stmt)
{
	...
	if (slot == NULL)
		return csvarmap[cs_create_variable_info_for (t, alias_get_name (t]));
...
asprintf (&tempname, "%s.%d", name,  csvarmap.length ());
newname = ggc_strdup (tempname);
		...
		bool address_p, bool lhs_p)
{
	...
	if (result->type == SCALAR
			&& csvarmap[result->var]->is_full_var)
		...
		if ((unsigned HOST_WIDE_INT)bitpos < csvarmap[result->var]->fullsize
				&& bitmaxsize != 0) {
			...
			cexpr.offset = 0;
			for (curr = csvarmap[cexpr.var]; curr; curr = curr->next) {
...
/* Find the first varinfo in the same variable as START that overlaps with
   OFFSET.  Return NULL if we can't find one.  */
	static csvarinfo_t
cs_first_vi_for_offset (csvarinfo_t start, unsigned HOST_WIDE_INT offset)
{
	offset += start->offset;
...
/* Handle aggregate copies by expanding into copies of the respective
   fields of the structures.  */
	static void
cs_do_structure_copy (tree lhsop, tree rhsop)
{
	struct constraint_expr *lhsp, *rhsp;
	vec <ce_s, heap> lhsc, rhsc;
	unsigned j;
	...
	if (rhsp->type == DEREF) {
		...
		for (j = 0; VEC_iterate (ce_s, lhsc, j, lhsp);) {
			...
			rhsp =  rhsc.index ( k);
			lhsv = csvarmap[lhsp->var];
			rhsv = csvarmap[rhsp->var];
...
check_callstring_existance (callstring cs, basic_block bb, bool in_ipdfa)
{
	...
	for (; ai; ai=ai->next) {
		callstring bi =  callstrings.index ( ai->cs_index);
...
check_alias_inclusion (constraint_t t, csvarinfo_t vi, unsigned int *loc)
{
	...
	for (ai=vi->constraints_with_vi_as_lhs; ai; ai=ai->next) {
		if (constraint_equal (*t, * aliases.index ( ai->val))) {
			*loc = ai->val;
			return true;
		}
	}
...
cs_process_constraint (constraint_t t)
{
	...
	if (!compute_only_pinfo)
		insert_alias_in_pool (t, csvarmap[lhs.var]);
...
preprocess_cfg (void)
{
	...
	for (cnode=cgraph_nodes; cnode; cnode=cnode->next) {
		...
		//fprintf(dump_file, "in preprocess_cfg11\n");
...
{
	callstring bi = (index == -1) ? NULL :  callstrings.index ( index);
...
print_callstring_sequence (int cs)
{
	if (cs == -1)
		...
		callstring ci =  callstrings.index ( cs);
		fprintf (dump_file, "{");
		for (; ci; ci=ci->next) {
		...
		basic_block succ_bb, bool in_ipdfa)
{
	...
	if ((*ai)->cs_index == -1)
		...
		oldcs =  callstrings.index ( (*ai)->cs_index);
	newcs = (callstring) alloc_mem (sizeof (struct callstring_def));
	newcs->bb = bb;
	newcs->next = oldcs;
	index = check_callstring_existance (newcs, succ_bb, in_ipdfa);
	if (index == -1) {
		...
		if (repcs == -1) { 
			...
			callstring oldrep =  callstrings.index ( repcs);
...
{
	int index = -1;
	callstring oldcs =  callstrings.index ( cs);
		...
		print_pointsto_info (basic_block bb)
		{
			...
			if (start_block (bb)) {
				for (in=in_of_bb (bb); in; in=in->next) {
					...
					fprintf (dump_file, "\nmayin : ");
					/*in_info = (in->pinfo < 0) ? NULL :  mayinfo.index ( in->pinfo);*/
					print_pointsto_list (in->pinfo);
					fprintf (dump_file, "\nmayout : ");
					if (out) {
						/*out_info = (out->pinfo < 0) ? NULL :  mayinfo.index ( out->pinfo);*/
		...
		print_liveness_info (basic_block bb)
		{
			...
			if (start_block (bb)) {
				for (in=in_of_bb (bb); in; in=in->next) {
					...
					fprintf (dump_file, "\nliveout : ");
					if (out) {
						/*out_info = (out->linfo < 0) ? NULL :  liveinfo.index ( out->linfo);*/
						print_df_list (out->linfo);
					}
					else 
						fprintf (dump_file, "value is mapped"); 
					fprintf (dump_file, "\nlivein : ");
					/*in_info = (in->linfo < 0) ? NULL :  liveinfo.index ( in->linfo);*/
		...
		mark_live_at_endblock (basic_block bb, unsigned int lhs)
		{
			...
			for (ai=out_of_bb (bb); ai; ai=ai->next) {
				if (ai->cs_index == cpinfo->cs_index) {
					/*df_list out = (ai->linfo < 0) ? NULL : copy_df_list ( liveinfo.index ( ai->linfo));*/
					old_linfo = ai->linfo;
	...
	static bool 
				...
				HOST_WIDE_INT lhsoffset, unsigned int rhsvar, HOST_WIDE_INT rhsoffset) 
		{
			...
			if (!unexpandable_var (lhsvar, lhsoffset)) {
				lhs_vi = cs_first_vi_for_offset (csvarmap[lhsvar], lhsoffset);
				if (lhs_vi) {
					lhsvar = lhs_vi->id;
					lhsoffset = 0;
				}
			...
			if (!unexpandable_var (rhsvar, rhsoffset)) {
				rhs_vi = cs_first_vi_for_offset (csvarmap[rhsvar], rhsoffset);
				if (rhs_vi) { 
					rhsvar = rhs_vi->id;
					rhsoffset = 0;
				}
	...
	static bool 
				...
				struct constraint_expr lhs, struct constraint_expr rhs)
		{
			/*pointsto_val in = (index < 0) ? NULL :
			  copy_pointsto_val ( tmpinfo.index ( index));
			  pointsto_val ai;*/
	...
	static int 
		...
		static void
			compute_stmt_out_1 (csdfa_info dfainfo, constraint_t t)
			{
				...
				/*in = copy_pointsto_val ( mayinfo.index ( dfainfo->pinfo));*/
						...
						if (ci->new_ptadf) {
							...
							for (bi=get_constraint_start (cbb); bi; bi=bi->next) {
								...
								if (!bi->stmt_constraint_chain->next) {
									constraint_t temp =  aliases.index (bi->stmt_constraint_chain->val);
								...
								/* We are dealing with constraints generated from PHI nodes. */
									...
									for (ai=bi->stmt_constraint_chain; ai; ai=ai->next) {
										...
										constraint_t temp =  aliases.index ( ai->val);
		...
		/*static void
		  ...
		  df_list lin = (index < 0) ? NULL :  liveinfo.index ( index);
		  if(dump_file && !ipacs_time) {
		  fprintf(dump_file,"\n\n%s : ",s);
		  print_df_list(lin);
		  }
				...
				for (ai=in_of_bb (caller_bb); ai; ai=ai->next) {
					if (ai->new_ptadf) {
						...
						if (ci && (ci->cs_index == ai->cs_index)) {
							/*pinfo = (ai->pinfo < 0) ? NULL :  mayinfo.index ( ai->pinfo)
					...
					static bool
						remove_from_df_list (unsigned int val, int *list)
						{
							...
							df_list bi  = (*list < 0) ? NULL : copy_df_list (  liveinfo.index ( *list));
					...
					static void
								...
								int *gen, int *kill, int linfo)
						{
							...
							for (list=bi->stmt_constraint_chain; list; list=list->next) {
								constraint_t t =  aliases.index ( list->val);
								struct constraint_expr lhs = t->lhs;
								struct constraint_expr rhs = t->rhs;
								bool lhs_live = false;
								int lhsvar = -1;
								...
								if (init && lhs.type == SCALAR) {
									...
									if (!unexpandable_var (lhs.var, lhs.offset)) {
										lhs_vi = cs_first_vi_for_offset (csvarmap[lhs.var], lhs.offset);
										...
										*gen = *kill = -1;
										...
										if (must_lhs) {
											...
											if (!unexpandable_var (dirval, lhs.offset)) {
												csvarinfo_t lhs_vi = NULL;
												lhs_vi = cs_first_vi_for_offset (csvarmap[dirval], lhs.offset);
												if (lhs_vi)
													lhsvar = lhs_vi->id;
												else{
													/*lhsvar = di->rhs->val;*/
					...
					static int
						compute_live_parms (int index)
						{
							...
							for (bi=get_constraint_end (cbb); bi; bi=bi->prev) {
								if (bi->is_alias) {
									constraint_t t =  aliases.index ( bi->stmt_constraint_chain->val);
							...
							for (ai=out_of_bb (cbb); ai; ai=ai->next) {
								if (ai->new_lvdf == true) {
									...
									if (lhsc) {
										...
										if (bi->linfo >= 0) {
											int temp = bi->linfo;
											/*df_list temp = (copy_df_list ( liveinfo.index ( bi->linfo)));*/
					...
					/****************************** MY_CHANGES STARTS HERE ******************************/

					FILE *fp_dump;

					static bool is_pointer_var(tree);
					...
					/* Dump bitmap for variable 'var' to string 'str' */

					static void
						dump_bitmap_to_string (char *str, tree var, pointsto_val pval)
						{
							int index = 0;
							char str1[100] = "\0";

							while(pval) {
								if ( var == ( ( csvarmap.index ( pval->lhs))->decl )) {
									//                        fprintf(stdout,"\nMatch: %s\n", (( csvarmap.index ( pval->lhs))->name));
									bitmap_iterator bir;
									unsigned int rhsbit;
									...
									{
										sprintf (str1, "%s -> %s ",csvarmap[pval->lhs]->name, csvarmap.index(rhsbit)->name);
										strcat(str, str1);
										if(i++ < cnt)
											strcat (str, ", ");
									}
					...
					/* Helps function 'dump_lipta_info_for_var' for dumping the LIPTA poinsto info for variable 'var' */

					static bool
						dump_lipta_info_for_var_1 (tree var, pointsto_val pval, bool is_ssa_name_var = false)
						{
							int index = 0;
							pointsto_val pval_start = pval;

							bool flag=false, is_ref = false;
							char str[500] = "\0";
							...
							if (is_ssa_name_var == true) {
								var = SSA_NAME_VAR(var);
								if (var == NULL) {
									fprintf(fp_dump,"\n SSA_NAME_VAR null. Returning....");
									return true;
								}
							}
							...
							while(pval) {
								if ( var == ( ( csvarmap.index ( pval->lhs))->decl )) {
									...
									EXECUTE_IF_SET_IN_BITMAP(pval->rhs,0,rhsbit,bir)
									{
										fprintf (fp_dump, "%s -> %s",csvarmap[pval->lhs]->name, csvarmap.index(rhsbit)->name);
										if(i++ < cnt)
											fprintf (fp_dump, ", ");
										if (is_ref) {
											dump_bitmap_to_string (str, (csvarmap[rhsbit])->decl, pval_start);
										}
									}
									fprintf(fp_dump," }\n");
							...
							if(!flag) {
								print_generic_expr(fp_dump,var,0);
								fprintf(fp_dump,": No pointsto info Found in pointsto_val LHS linked list.\n");
								return false;
							}
							if (is_ref) {
								//dump string 'str' storing points to info for reference
								fprintf(fp_dump, "*%s pointsto { %s }\n",get_name(var), str);
							}
							return true;
						}

					/* Dumps the LIPTA poinsto info for variable 'var' using function 'dump_lipta_info_for_var_1' */

					static void
						dump_lipta_info_for_var (tree var, pointsto_val in_pval, pointsto_val out_pval)
						{
							int i;
							pointsto_val pval;

							//	fprintf(stdout,"\nHere %s\n",(TREE_CODE (var) == POINTER_TYPE ? "*" : "&"));

							// IN pointsto information
							if (in_pval) {
								fprintf(fp_dump, "\nIN: ");

								if ( ! dump_lipta_info_for_var_1 (var, in_pval) ) {
									if (SSA_NAME_VAR(var)) {
										fprintf(fp_dump, "IN: SSA_NAME_VAR: "); //%s",get_name(SSA_NAME_VAR(var)));
										dump_lipta_info_for_var_1 (var, in_pval, true);
									}
									else
										fprintf(fp_dump, "\nERROR:: IN: SSA_NAME_VAR: NULL\n");
								}
							}
							else
								fprintf(fp_dump,"\nERROR: IN: No IN info for %s : IN pointosto_val empty.\n", get_name(var));

							// OUT pointsto information
							if (out_pval) {
								fprintf(fp_dump, "\nOUT: ");

								if ( ! dump_lipta_info_for_var_1 (var, out_pval) ) {
									if (SSA_NAME_VAR(var)) {
										fprintf(fp_dump, "OUT: SSA_NAME_VAR: "); //%s",get_name(SSA_NAME_VAR(var)));
										dump_lipta_info_for_var_1 (var, out_pval, true);
									}
									else
										fprintf(fp_dump, "\nERROR:: OUT: SSA_NAME_VAR: NULL\n");
								}
							}
							else
								fprintf(fp_dump,"\nERROR: OUT: No OUT info for %s : OUT pointosto_val empty.\n", get_name(var));
						}

					/*---------------------------------------------------------
					  Dumps flow-sensitive information in LIPTA data structures
					  for given gimple statement 'stmt' to file 'fp_dump'
					  -----------------------------------------------------------*/

					static void
						dump_lipta_info_for_stmt (gimple stmt, pointsto_val in_pval, pointsto_val out_pval)
						{
							tree lhsop = gimple_assign_lhs (stmt);
							tree rhsop = gimple_assign_rhs1 (stmt);

							fprintf(fp_dump, "\nLHSOP: %s\n", get_name(lhsop));
							dump_lipta_info_for_var (lhsop, in_pval, out_pval);

							fprintf(fp_dump, "\nRHSOP: %s\n", get_name(rhsop));
							if (rhsop)
								//              if (is_pointer_var (rhsop))
								dump_lipta_info_for_var (rhsop, in_pval, out_pval);

							//              else
							//                      fprintf(fp_dump, "ERROR: RHS %s is not a pointer!!", get_name(rhsop));
						}

					/*--------------------------------------------DUMP LIPTA INFO ENDS---------------------------------------------------------*/

					/*
					   void
					   my_dump_points_to_solution (FILE *file, struct pt_solution *pt)
					   {
					   if (pt == NULL)
					   fprintf(stdout,"##############################################################");
					   if (pt->anything)
					   ...
					   void
					   my_dump_points_to_info_for (FILE *file, tree ptr)
					   {
					   ...
					   if (pi)
					   my_dump_points_to_solution (file, &pi->pt);
					   else
					   fprintf (file, ", points-to anything");
					...
					/*---------------------------------------------------------
					  Dumps flow-insensitive information in SSA_NAME_PTR_INFO
					  for given gimple statement 'stmt' to file 'fp_dump'
					  -----------------------------------------------------------*/

					static void
						dump_ssa_name_ptr_info_for_stmt (gimple stmt)
						{
							tree lhsop = gimple_assign_lhs (stmt);
							tree rhsop = gimple_assign_rhs1 (stmt);
							...
							dump_points_to_info_for(fp_dump, lhsop);
							/*
							   if (REFERENCE_CLASS_P(rhsop)) {
							   rhsop = TREE_OPERAND (rhsop, 0);
							   is_ref = true;
							   }

							   if (rhsop)
							//		if (is_pointer_var (rhsop))			//check not necessary, it has to be a pointer
							dump_points_to_info_for(fp_dump, rhsop);

							if (rhsop && is_ref) {
							//		if (is_pointer_var (rhsop)) {
							fprintf(fp_dump, "*");
							print_generic_expr (fp_dump, rhsop, 0);
							fprintf (fp_dump, ", points-to { ");

							struct ptr_info_def *pi = SSA_NAME_PTR_INFO (rhsop);

							if (pi && (&pi->pt)->vars) {
							bitmap_iterator bi;
							unsigned i;

							EXECUTE_IF_SET_IN_BITMAP ((&pi->pt)->vars, 0, i, bi) {
							tree var = referenced_var_lookup (cfun, i);
							if (var) {
							struct ptr_info_def *pi = SSA_NAME_PTR_INFO (var);
							if (pi)
							dump_points_to_solution(fp_dump, &pi->pt);
							else
							fprintf (fp_dump, ", points-to anything");
							}
							else
							fprintf(fp_dump, "\nERROR: In dump_SSA_NAME_PTR_INFO: In else- ref_var_lookup failed..\n");
							}
							}
							fprintf (fp_dump, " }\n");
							//		}
					...
					/*--------------------------------------------DUMP SSA_NAME_PTR_INFO ENDS---------------------------------------------------------*/

					/*--------------------------------------------INJECT LIPTA INFO INTO GCC---------------------------------------------------------*/

					/* Determines bitmap for a variable 'var'. Here vaiable is a regualr pointer */

					static bool
						set_bitmap_for_var (tree var, pointsto_val pval, bitmap vars, bool *pointsto_global, bool *pointsto_null, bool *pointsto_undef)
						{
							int index = 0;
							...
							while(pval) {
								if ( var == ( ( csvarmap.index ( pval->lhs))->decl )) {
									...
									EXECUTE_IF_SET_IN_BITMAP(pval->rhs,0,rhsbit,bir)
									{
										if ((csvarmap[rhsbit])->name == "undef") {
											*pointsto_undef = true;
											continue;
										}
										if ((csvarmap[rhsbit])->name == "null") {
											*pointsto_null = true;
											continue;
										}

										unsigned int uid = DECL_PT_UID ((csvarmap[rhsbit])->decl);

										gcc_assert (DECL_P ( (csvarmap[rhsbit])->decl ) );
										//                     			&& (!optimize
										//                     				|| referenced_var_lookup (cfun, DECL_UID ( (csvarmap[rhsbit])->decl ))));
										//last condition in assert gives seg fault for global variables

										//				fprintf(stdout,"\nSetting bit for %s\n",(csvarmap[rhsbit])->name);
										bitmap_set_bit (vars, uid);

										if (is_global_var((csvarmap[rhsbit])->decl))
											*pointsto_global = true;
									}
									return true;
								}
								pval=pval->next;
					...
					/* Determines pointsto bitmap for a variable 'var' which can be a reference or a regualr pointer */

					static bool
						set_bitmap_for_var_ref (tree var, pointsto_val pval, bitmap vars, bool is_ref, bool *pointsto_global, bool *pointsto_null, bool *pointsto_undef)
						{
							int index = 0;
							pointsto_val pval_start = pval;

							if (!is_ref)
								return set_bitmap_for_var (var, pval, vars, pointsto_global, pointsto_null, pointsto_undef);
							...
							while(pval) {
								if ( var == ( ( csvarmap.index ( pval->lhs))->decl )) {
									...
									EXECUTE_IF_SET_IN_BITMAP(pval->rhs,0,rhsbit,bir)
									{
										set_bitmap_for_var (((csvarmap[rhsbit])->decl), pval_start, vars,pointsto_global,pointsto_null,pointsto_undef);
									}
									return true;
								}
								pval=pval->next;
					...
					/*-------------------------------------------------------
					  Insert flow-sensitive information of gimple statement 'stmt'
					  from LIPTA into SSA_NAME_PTR_INFO
					  -------------------------------------------------------*/

					static void
						insert_lipta_info_into_gcc (gimple stmt, pointsto_val out_pval)
						{
							tree lhsop = gimple_assign_lhs (stmt);
							tree rhsop = gimple_assign_rhs1 (stmt);

							bitmap vars = BITMAP_GGC_ALLOC (), old_vars;
							bool is_ref = false, pointsto_global = false, pointsto_null = false, pointsto_undef = false;

							struct ptr_info_def *pi = get_ptr_info (lhsop);

							if (!pi)
								fprintf(fp_dump,"\nERROR:: NO ptr_info_def for %s.. Wrong definition of relevant statements..\n",get_name(lhsop));
							...
							if (! set_bitmap_for_var_ref (rhsop, out_pval, vars, is_ref, &pointsto_global, &pointsto_null, &pointsto_undef) )
								if (! set_bitmap_for_var_ref (SSA_NAME_VAR(rhsop), out_pval, vars, is_ref, &pointsto_global, &pointsto_null, &pointsto_undef) )
									fprintf(fp_dump, "\nError: No info found at all for %s...",get_name(rhsop));

							if (! bitmap_equal_p (old_vars, vars)) {
								fprintf(fp_dump,"\nSUCCESS: DIFFERENT BITMAPS......\n");
								pt_solution_set (&pi->pt, vars, pointsto_global);
					...
					/*--------------------------------------------INJECT LIPTA INFO INTO GCC ENDS---------------------------------------------------------*/

					static bool
						is_relevant_stmt (gimple stmt)
						{
							tree lhsop = NULL;
							...
							if (is_gimple_assign (stmt)) {
								...
								if ((is_pointer_var(lhsop)) & (DECL_ARTIFICIAL(lhsop)) & (REFERENCE_CLASS_P(lhsop))) {
									fprintf(fp_dump,"\nDISCARDING STATEMENT (LHS is a reference):\n");
									print_gimple_stmt (fp_dump, stmt, 0, 0);
									fprintf(fp_dump, "\n-------------------------------------------------------------------------------------\n");
								}

								return ((is_pointer_var(lhsop)) & (DECL_ARTIFICIAL(lhsop)) & (!(REFERENCE_CLASS_P(lhsop))));

								//		if ( (!(REFERENCE_CLASS_P(lhsop))) && (!(REFERENCE_CLASS_P(gimple_assign_rhs1(stmt)))))
							}
							return false;
						}

					static void
						dump_stmt_info (gimple stmt)
						{
							tree lhsop = gimple_assign_lhs (stmt);
							tree rhsop = gimple_assign_rhs1 (stmt);
							...
							if (TREE_CODE(lhsop) != SSA_NAME)
								fprintf(fp_dump,"\n ERROR :: LHS is not a SSA_NAME for %s...............................",get_name(lhsop));

							if (TREE_CODE(rhsop) != SSA_NAME)
								fprintf(fp_dump,"\n ERROR :: RHS is not a SSA_NAME for %s",get_name(rhsop));

							if (TREE_CODE(SSA_NAME_VAR(lhsop)) != SSA_NAME)
								fprintf(fp_dump,"\n ERROR :: LHS SSA_NAME_VAR is not a SSA_NAME for %s",get_name(SSA_NAME_VAR(lhsop)));

							rhsop = SSA_NAME_VAR (rhsop);

							if (rhsop != NULL && ((TREE_CODE(rhsop)) != SSA_NAME) )
								fprintf(fp_dump,"\n ERROR :: RHS SSA_NAME_VAR is not a SSA_NAME for %s", get_name(rhsop));
							else if (rhsop == NULL)
								fprintf(fp_dump, "\n ERROR :: RHS SSA_NAME_VAR IS NULL\n");

							fprintf(fp_dump,"\n");
						}

					/*-----------------------------------------------------------------------------
					  Inserts the pointsto information computed by LIPTA into gcc global
					  data structure SSA_NAME_PTR_INFO or more precisely, to pt_solution.

					  Also it displays the relevant information from LIPTA and gcc data structures.
					  ------------------------------------------------------------------------------*/

					static void
						inject_pointsto_info (void)
						{
							struct cgraph_node *node;
							basic_block bb;
							gimple_stmt_iterator gsi;
							...
							pointsto_val in_pval, out_pval;

							/*	if ( !(fp_dump = fopen("/home/nikhil/MTech_Spring_13/CS715-GCC/Project/modified_lipta/dump","w")) ) {
								fprintf(stderr, "Dump file could not be opened!! Using LIPTA dump..");
								fp_dump = dump_file;
								}
								*/	fp_dump = stdout;
							...
							for (node = cgraph_nodes; node; node=node->next) {
								...
								fprintf(fp_dump, "\n ***Function %s*** %u\n",cgraph_node_name(node), cfun->gimple_df->ipa_pta);

								FOR_EACH_BB (bb) {
									//find in_ipdfa and out_ipdfa for block
									...
									if (bb->aux != NULL) {
										...
										if (in_csdfa != NULL && (in_csdfa->pinfo) >= 0 ) {
											in_pval =  mayinfo.index ( in_csdfa->pinfo);
										}
										out_csdfa = out_of_bb(bb);
										if (out_csdfa != NULL && (out_csdfa->pinfo) >= 0 ) {
											out_pval =  mayinfo.index ( out_csdfa->pinfo);
										}
									}
									...
									for (gsi=gsi_start_bb (bb); !gsi_end_p (gsi); gsi_next (&gsi)) {
										...
										// inject the information if statement is relevant
										if (is_relevant_stmt (stmt)) {
											...
											//make sure that lhs is a SSA_NAME
											gcc_assert(TREE_CODE( gimple_assign_lhs(stmt)) == SSA_NAME );

											//print flow-insensitive information (info in SSA_NAME_PTR_INFO)
											fprintf(fp_dump, "\n*****PTA*****\n");
											...
											//insert flow sensitive LFAPA info into SSA_NAME_PTR_INFO, the gcc structure for storing pointsto info
											//					fprintf(fp_dump, "\n*****Insertion of LIPTA info into gcc*****\n");
											//					insert_lipta_info_into_gcc (stmt, out_pval);
										...
										print_gimple_stmt (fp_dump, gsi_stmt(gsi), 0, 0);
										}
										*/
									}
								}
