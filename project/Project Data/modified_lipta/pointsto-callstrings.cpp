...
static std::map <unsigned int, tree> uid_to_tree;
/*-----------------------------------------------------------------------------
...
static inline csvarinfo_t cs_get_varinfo (unsigned int n);
static bool set_bitmap_for_var (tree var, pointsto_val pval, bitmap vars, bool *pointsto_global, bool *pointsto_null, bool *pointsto_undef);
static bool set_bitmap_for_var_ref (tree var, pointsto_val pval, bitmap vars, bool is_ref, bool *pointsto_global, bool *pointsto_null, bool *pointsto_undef);
/*----------------------------------------------------------------------
	...
	for (i = 0; i < num_ssa_names; i++)
	{
		...
		if (pi)
		{
			...
			if (pt->vars)
			{
				...
				{
					if (uid_to_tree[rhsbit]) 
						fprintf (dump_file, " %s", IDENTIFIER_POINTER (DECL_NAME (uid_to_tree [rhsbit])));
					else 
		...
		if (gimple_has_body_p(node->decl))
		{
			...
			for (i = 0; i < num_ssa_names; i++)
			{
				...
				if (pi)
				{
					...
					(&pi->pt)->vars = vars;
					fprintf (dump_file, "UID: %d, ", DECL_UID (SSA_NAME_VAR (ptr)));
					print_generic_expr (dump_file, SSA_NAME_VAR (ptr), 0);
			...
			if (dump_file && bb)
			{
				...
				for(lin = in_of_bb(bb); lin; lin=lin->next)
				{
					...
					for(lout = out_of_bb(bb); lout; lout=lout->next)
					{
						...
						{
							fprintf(dump_file,"\nCallstring : ");
							print_callstring_sequence(lin->cs_index);
							fprintf(dump_file,"\nlivein : ");
							print_df_list(lin->linfo);

							fprintf(dump_file,"\nCallstring : ");
							print_callstring_sequence(lout->cs_index);
							fprintf(dump_file,"\nliveout : ");
							print_df_list(lout->linfo);
							break;
						}
...
dump_bitmap_to_string (char *str, tree var, pointsto_val pval)
{
	...
	while(pval)
	{
		if ( var == ( (VEC_index (csvarinfo_t, csvarmap, pval->lhs))->decl ))
		{
//						fprintf(stdout,"\nMatch: %s\n", ((VEC_index(csvarinfo_t, csvarmap, pval->lhs))->name));
			bitmap_iterator bir;
			unsigned int rhsbit;
			...
			{
				sprintf (str1, "%s -> %s ",cs_get_varinfo (pval->lhs)->name, cs_get_varinfo (rhsbit)->name);
				strcat(str, str1);
				if(i++ < cnt)
					strcat (str, ", ");
			}
...
//						fprintf(stdout,"\nMatch: %s\n", ((VEC_index(csvarinfo_t, csvarmap, pval->lhs))->name));
			...
			EXECUTE_IF_SET_IN_BITMAP(pval->rhs,0,rhsbit,bir)
			{
				fprintf (fp_dump, "%s -> %s",cs_get_varinfo (pval->lhs)->name, cs_get_varinfo (rhsbit)->name);
				if(i++ < cnt)
					fprintf (fp_dump, ", ");
				if (is_ref)
				{
					dump_bitmap_to_string (str, (cs_get_varinfo (rhsbit))->decl, pval_start);
				}
			}
			fprintf(fp_dump," }\n");
...
set_bitmap_for_var (tree var, pointsto_val pval, bitmap vars, bool *pointsto_global, bool *pointsto_null, bool *pointsto_undef)
{
	...
	while(pval)
	{
		if ( var == ( (VEC_index(csvarinfo_t, csvarmap, pval->lhs))->decl ))
		{
			...
			EXECUTE_IF_SET_IN_BITMAP(pval->rhs,0,rhsbit,bir)
			{
				if ((cs_get_varinfo (rhsbit))->name == "undef")
				{
					*pointsto_undef = true;
					continue;
				}
				if ((cs_get_varinfo (rhsbit))->name == "null")
				{
					*pointsto_null = true;
					continue;
				}

				unsigned int uid = DECL_PT_UID ((cs_get_varinfo (rhsbit))->decl);
...
//					 			&& (!optimize
//					 				|| referenced_var_lookup (cfun, DECL_UID ( (cs_get_varinfo (rhsbit))->decl ))));
				//last condition in assert gives seg fault for global variables

//				fprintf(stdout,"\nSetting bit for %s\n",(cs_get_varinfo (rhsbit))->name);
				bitmap_set_bit (vars, uid);

				if (is_global_var((cs_get_varinfo (rhsbit))->decl))
...
set_bitmap_for_var_ref (tree var, pointsto_val pval, bitmap vars, bool is_ref, bool *pointsto_global, bool *pointsto_null, bool *pointsto_undef)
{
	...
	while(pval)
	{
		if ( var == ( (VEC_index(csvarinfo_t, csvarmap, pval->lhs))->decl ))
		{
			...
			EXECUTE_IF_SET_IN_BITMAP(pval->rhs,0,rhsbit,bir)
			{
				set_bitmap_for_var (((cs_get_varinfo (rhsbit))->decl), pval_start, vars,pointsto_global,pointsto_null,pointsto_undef);
			}
			return true;
		}
		pval=pval->next;
...
#if 0
static void
insert_lipta_info_into_gcc_forphi(gimple stmt,pointsto_val out_pval)
{
	//if(!(gimple_code (stmt) == GIMPLE_PHI))
	// fprintf (stderr, "in insert_lipta_info_into_gcc\n");
	tree lhsop = gimple_assign_lhs (stmt);
	tree rhsop = gimple_assign_rhs1 (stmt);
...
#endif
static bool
is_relevant_stmt_phi (tree lhsop)
{
//tree lhsop = gimple_assign_lhs (stmt);
...
// tree phi_result = gimple_phi_result (phi);
	if (is_gimple_reg (lhsop))
	{
		if (field_must_have_pointers (lhsop))
		{
			...
			fprintf(fp_dump," is relevant stmt\n");
			return true;
		}
	}
	return false;
...
is_relevant_stmt (gimple stmt)
{
	...
	fprintf(fp_dump,"in is_relevant");
	if (is_gimple_assign (stmt))
	{
...
//		if(field_must_have_pointers(lhsop)) return true;
		...
		//	return ( (!(REFERENCE_CLASS_P(lhsop))) && ((REFERENCE_CLASS_P(gimple_assign_rhs1(stmt)))));
...
//			fprintf(fp_dump, "\n:::::::::::New block::::::::::\n");
			...
			for (gsi=gsi_start_bb (bb); !gsi_end_p (gsi); gsi_next (&gsi))
			{
				...
				fprintf(fp_dump,"in here\n");
				//if(!(gimple_code (stmt) == GIMPLE_PHI)){
				//fprintf(fp_dump,"not a phi stmt\n");
				//fprintf(fp_dump,"in here\n");
				if (is_relevant_stmt (stmt))
				{
					...
					fprintf(fp_dump," in relevant stmt.");
					fprintf(fp_dump, "\n");
					// print_gimple_stmt (fp_dump, stmt, 0, 0);
...
//					dump_ssa_name_ptr_info_for_stmt (stmt);
			...
			{
				gimple phi = gsi_stmt(gsi);
				print_gimple_stmt(fp_dump,phi, 0, 0);
				tree lhsop=gimple_phi_result(phi);
				print_generic_stmt(fp_dump,lhsop,0);
				if (is_relevant_stmt_phi (lhsop))
				{
					//tree lhsop=gimple_phi_result(phi);
					//insert_lipta_info_into_gcc (phi, out_pval);
				}
			}
