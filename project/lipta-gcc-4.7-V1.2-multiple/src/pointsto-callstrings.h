...
static vec<csvarinfo_t> csvarmap;
//DEF_VEC_P(constraint_t);
//DEF_VEC_ALLOC_P(constraint_t,heap);
static vec<constraint_t> aliases;
//DEF_VEC_P(pointsto_val);
//DEF_VEC_ALLOC_P(pointsto_val, heap);
static vec<pointsto_val> mayinfo;
static vec<pointsto_val> tmpinfo;
//DEF_VEC_P(callstring);
//DEF_VEC_ALLOC_P(callstring, heap);
static vec<callstring> callstrings;
//DEF_VEC_P(df_list);
//DEF_VEC_ALLOC_P(df_list, heap);
static vec<df_list> liveinfo;
...
bool push_fields_onto_fieldstack (tree, vec<fieldoff_s> &, HOST_WIDE_INT);
bool check_for_overlaps (vec<fieldoff_s> &);
void sort_fieldstack (vec<fieldoff_s> &);

static void cs_get_constraint_for_address_of (tree, vec<ce_s> &);
static void cs_get_constraint_for_rhs (tree, vec<ce_s> &);
static void cs_get_constraint_for_1 (tree, vec<ce_s> &, bool, bool);
        ...
        if (push)
          {
            ...
            if (pair
            ...
            else
              {
                ...
                pair = fieldstack.safe_push (*pair);
              }
          }
...
{
	unsigned int i;
   	for (i=0; i<liveinfo.length (); i++) 
	{
       if (df_list_equal (a, liveinfo[i]))
           return i;
   	}
   return -1;
}
...
compute_liveness_index (df_list in)
{
   	...
   	else 
	{
       	...
       	if (index < 0) 
		{
           index = liveinfo.length ();
           liveinfo.safe_push (in);
		}
   }
   return index;
}
...
index_in_maypool (pointsto_val a)
{
 ...
 unsigned int i;
  for (i=0; i<mayinfo.length (); i++) {
       if (pointsto_val_equal (a, mayinfo[i]))
           return i;
   }
   return -1;
}
...
index_in_tmppool (pointsto_val a)
{
   ...
   for (i=0; i<tmpinfo.length (); i++) {
       if (pointsto_val_equal (a, tmpinfo[i]))
           return i;
   }
   return -1;
}
...
{
int index =-1;
 /*pointsto_val out = (index < 0) ? NULL : mayinfo[index];*/
...
compute_index_in_tmppool(pointsto_val out)
{
   ...
   /*pointsto_val out = (index < 0) ? NULL : tmpinfo[ index];*/
int index =-1;
   if (out == NULL)
       return -1;
   else {
       ...
       if (index < 0) {
           index = tmpinfo.length ();
           tmpinfo.safe_push (out);
       }
   }
   return index;
...
get_pointee_set(int in_index,vec_list list, unsigned int varid)
{
 ...
 /*printf("\nget_pointee_set\n");*/
  ...
  if(list == tmplist)
  in = (in_index < 0) ? NULL :
                         copy_pointsto_val (tmpinfo[in_index]);
  if(list == maylist)
  in = (in_index < 0) ? NULL :
                         copy_pointsto_val (mayinfo[ in_index]);
...
has_must_pointee(int in_index, vec_list list, unsigned int varid)
{
 /*printf("\nhas_must_pointee\n");*/
  ...
  pointsto_val ai,in= NULL;
  if(list == tmplist)
  	in = (in_index < 0) ? NULL :  copy_pointsto_val ( tmpinfo [ in_index]);
  if(list == maylist)
        in = (in_index < 0) ? NULL :  copy_pointsto_val ( mayinfo [ in_index]);
...
bool is_last = false, is_equal = false;
	...
	if(list == maylist){
               in = (index < 0) ? NULL :
                        copy_pointsto_val ( mayinfo [ index]);

	}
	else if(list == tmplist) {
                in = (index < 0) ? NULL :
                         copy_pointsto_val ( tmpinfo [ index]);
	}
...
is_pointee_list_empty(int index)
{
    ...
    pointsto_val in = (index < 0) ? NULL :  tmpinfo [ index];
    if(in == NULL)
	return true;
    else
	return false;
...
get_bitmap_for_lhs(int index , vec_list list)
{
  /*printf("\ngeT_bitmap_lhs\n");*/
   ...
   if(list == maylist)	ai = (index < 0) ? NULL : copy_pointsto_val ( mayinfo [ index]);
   if(list == tmplist)  ai = (index < 0) ? NULL : copy_pointsto_val ( tmpinfo [ index]);
...
remove_pointee(int index,vec_list list,unsigned lhs,bitmap tmp_set)
{
	...
	pointsto_val in = NULL,ai;
        if(list == maylist) in = (index < 0) ? NULL : copy_pointsto_val( mayinfo [ index]);
        if(list == tmplist) in = (index < 0) ? NULL : copy_pointsto_val( tmpinfo [ index]);
	ai = in;
...
merge_pointsto_info(int in1, int in2, vec_list list)
{
   ...
   if (in1 == in2)
       return in1;
  if(list == maylist){
   in = (in1 < 0) ? NULL : copy_pointsto_val ( mayinfo [ in1]);
   update = (in2 < 0) ? NULL :  mayinfo [ in2];
  }
  else if(list == tmplist){
	in = (in1 < 0) ? NULL : copy_pointsto_val ( tmpinfo [ in1]);
    update = (in2 < 0) ? NULL :  tmpinfo [ in2];
  }
...
remove_var_from_live_set(int index, unsigned int var)
{
     ...
     df_list in =  (index < 0) ? NULL : copy_df_list ( liveinfo [ index]);
...
get_var_at_index_in_liveset(int lin, int index){
     df_list in =  (lin < 0) ? NULL : copy_df_list ( liveinfo [ lin]);
     int count = 0;
     while(in){
        if(index == count){
                return in->val;
...
is_var_live_bb_cs(basic_block bb, int callstring, unsigned int var, in_out io)
{
    ...
    df_list in = (ai->linfo < 0) ? NULL : copy_df_list (liveinfo[ ai->linfo]);
    while(in && in->val <= var)
    {
	if(in->val == var)
		return true;
...
is_var_live_bb(basic_block bb, unsigned int var, in_out io)
{
	...
	for(bi = ai; bi; bi=bi->next){
		found = false;
    		in = (bi->linfo < 0) ? NULL : copy_df_list (liveinfo[ bi->linfo]);
    		while(in && in->val <= var)
    		{
			if(in->val == var)
				found = true;
...
merge_liveness_info(int in1, int in2)
{
   ...
   if (in2 < 0 || in1 == in2)
	...
	in = (in1 < 0) ? NULL : copy_df_list ( liveinfo [ in1]);
	update = (in2 < 0) ? NULL : copy_df_list ( liveinfo [ in2]);
