...
#define VEC_index (T, X, I) ( X[I] )
#define VEC_length (T, X) ( X.length() )
/*-----------------------------------------------------------------------------
...
{
   return ((cgraph_info)(node->symbol.aux))->pcsmap;
}

static cs_map
lcsmap_of_cnode (struct cgraph_node *node)
{
   return ((cgraph_info)(node->symbol.aux))->lcsmap;
}

static bool
pcsmap_required (struct cgraph_node *node)
{
   return ((cgraph_info)(node->symbol.aux))->pcsmap_required;
}

static bool
lcsmap_required (struct cgraph_node *node)
{
   return ((cgraph_info)(node->symbol.aux))->lcsmap_required;
}

static basic_block 
get_bb_from_callstring (int cs)
{
   callstring ai = callstrings[ cs];
   return ai->bb;
}
...
{
   return ((cgraph_info)(node->symbol.aux))->bb_list;
}
...
end_bb_of_fn (struct cgraph_node *node)
{
   ...
   int i = n_basic_blocks_for_function (DECL_STRUCT_FUNCTION (node->symbol.decl)) - NUM_FIXED_BLOCKS;
   return bb_list[i].bb;
}
...
{
   return ENTRY_BLOCK_PTR_FOR_FUNCTION (DECL_STRUCT_FUNCTION (node->symbol.decl))->next_bb;
}
...
static void
cs_get_constraint_for_rhs (tree t, vec <ce_s> &results)
{
...
static void
cs_do_deref (vec <ce_s> &constraints)
{
...
/* Get a constraint expression vector from an SSA_VAR_P node.
   If address_p is true, the result will be taken its address of.  */
static void
cs_get_constraint_for_ssa_var (tree t, vec<ce_s> &results, bool address_p)
{
...
/* Efficiently generates constraints from all entries in *RHSC to all
   entries in *LHSC.  */
static void
cs_process_all_all_constraints (vec <ce_s> &lhsc, vec <ce_s> &rhsc)
{
...
/* Given a tree T, return the constraint expression for taking the
   address of it. */
static void
cs_get_constraint_for_address_of (tree t, vec <ce_s> &results)
{
   struct constraint_expr *c;
   unsigned int i;
...
/* Given a gimple tree T, return the constraint expression vector for it.  */
static void
cs_get_constraint_for (tree t, vec <ce_s> &results)
{
  gcc_assert ( results.length () == 0);
...
{
   struct constraint_expr *lhsp, *rhsp;
   vec<ce_s> lhsc = vec<ce_s> (), rhsc = vec <ce_s> ();
   unsigned j;
...
check_callstring_existance (callstring cs, basic_block bb, bool in_ipdfa)
{
   ...
   for (; ai; ai=ai->next) {
       callstring bi = callstrings[ ai->cs_index];
...
check_alias_inclusion (constraint_t t, csvarinfo_t vi, unsigned int *loc)
{
   ...
   for (ai=vi->constraints_with_vi_as_lhs; ai; ai=ai->next) {
       if (constraint_equal (*t, *aliases[ ai->val])) {
	   *loc = ai->val;
           return true;
       }
   }
			   ...
			   int *rp, int num_bb)
{
   ...
   node->symbol.aux = (void *) alloc_mem (sizeof (struct cgraph_info_def));
   for (i=0; i<num_bb; i++) {
       bb = BASIC_BLOCK (rp[i]);
       set_rp_index (bb, i);
       bb_list[i].bb = bb;
   ...
   ((cgraph_info)(node->symbol.aux))->bb_list = bb_list;
   ((cgraph_info)(node->symbol.aux))->pcsmap_required = false;
   ((cgraph_info)(node->symbol.aux))->lcsmap_required = false;
}
...
map_arguments_at_call (gimple stmt, tree decl, bool generate_liveness)
{
   ...
   if (generate_liveness) {
	   ...
	   tree arg = gimple_call_arg (stmt, j);
	       ...
	       vec<ce_s> results = vec<ce_s> ();
	       cs_get_constraint_for (arg, results);
	       FOR_EACH_VEC_ELT (results, i, exp)
...
associate_varinfo_to_alias (struct cgraph_node *node, void *data)
{
  if (node->alias || node->thunk.thunk_p)
    cs_insert_vi_for_tree (node->symbol.decl, (csvarinfo_t)data);
  return false;
}
...
init_info (void)
{
  // init_alias_heapvars ();
   ...
   FOR_EACH_FUNCTION (cnode) {
       ...
       if (!gimple_has_body_p (cnode->symbol.decl) || cnode->clone_of)
           continue;
       ...
       vi = cs_create_func_info_for (cnode->symbol.decl, cgraph_node_name (cnode));
...
preprocess_cfg (void)
{
   ...
   FOR_EACH_FUNCTION (cnode) {
       ...
       set_cfun (DECL_STRUCT_FUNCTION (cnode->symbol.decl));
       current_function_decl = cnode->symbol.decl;
       is_start_block = true;
       ...
       if (!endblock) 
           endblock = EXIT_BLOCK_PTR_FOR_FUNCTION (DECL_STRUCT_FUNCTION (cnode->symbol.decl))->prev_bb;
...
{
   callstring bi = (index == -1) ? NULL : callstrings[ index];
		           ...
		           basic_block succ_bb, bool in_ipdfa)
{
   ...
   if (index == -1) {
	   ...
	   callstring oldrep = callstrings[ repcs];
...
{
   int index = -1;
   callstring oldcs = callstrings[ cs];
...
print_pta_worklist (void)
{
   ...
   int i;
   int num_bb = n_basic_blocks_for_function (DECL_STRUCT_FUNCTION (cnode->symbol.decl)) - NUM_FIXED_BLOCKS;
       ...
       for (ci=out_of_bb (cbb); ci; ci=ci->next) {
		...
		for (bi=get_constraint_start (cbb); bi; bi=bi->next) {
		   ...
		   if (!bi->is_alias)
			...
			constraint_t temp = aliases[bi->stmt_constraint_chain->val];
			...
			int maygen_ind = -2;
                      ...
                      for (ai=bi->stmt_constraint_chain; ai; ai=ai->next) {
                           ...
                           constraint_t temp = aliases[ ai->val];
...
get_bb_for_lv_processing (bb_worklist list, bool *new_cnode)
{
   ...
   bool any_bit_set = false;
   int i = n_basic_blocks_for_function (DECL_STRUCT_FUNCTION (cnode->symbol.decl)) - NUM_FIXED_BLOCKS;
   for (; i>=0; i--) {
...
modify_pcsmap_1 (struct cgraph_node *node, cs_map bi, csdfa_info ai, cs_map prev_lhs)
{
   ...
   if (!ci) {
       ...
       if (bi->rhs)
           ((cgraph_info)(node->symbol.aux))->pcsmap_required = true;
   }
   /* If yes, merge the two rhs'es. */
       ...
       if (!prev_lhs)
           ((cgraph_info)(node->symbol.aux))->pcsmap = bi->next;
       else
...
modify_pcsmap_at_startblock (struct cgraph_node *node, csdfa_info ai, bool is_new)
{
   ...
   if (bi == NULL) {
       ...
       ((cgraph_info)(node->symbol.aux))->pcsmap = temp;
   }
   else {
...
modify_lcsmap_1 (struct cgraph_node *node, cs_map bi, csdfa_info ai, cs_map prev_lhs)
{
   ...
   if (!ci) {
       ...
       if (bi->rhs)
           ((cgraph_info)(node->symbol.aux))->lcsmap_required = true;
   }
   /* If yes, merge the two rhs'es. */
   else {
       csmap_rhs temp;
       ...
       /* Remove the row of bi. */
       if (!prev_lhs)
           ((cgraph_info)(node->symbol.aux))->lcsmap = bi->next;
       else
           prev_lhs->next = bi->next;
       ggc_free (bi);
       ((cgraph_info)(node->symbol.aux))->lcsmap_required = true;
...
modify_lcsmap_at_endblock (struct cgraph_node *node, csdfa_info ai, bool is_new)
{
   ...
   if (bi == NULL) {
       ...
       ((cgraph_info)(node->symbol.aux))->lcsmap = temp;
   }
   else {
       csmap_rhs temp = create_csmap_rhs (ai, is_present_in_out);
       temp->next = bi->rhs;
		   ...
		   EXECUTE_IF_SET_IN_BITMAP(pointee_set,0,cival,ci){
			   ...
			   continue;
		               ...
		               cpinfo = copy_csdfa_info (ai);
                               ...
                               map_arguments_at_call (stmt, node->symbol.decl, false);
...
mark_escaped_rhs (int index)
{
   bitmap_iterator bir;
   unsigned int var;
   pointsto_val root = (index < 0) ? NULL : copy_pointsto_val (mayinfo[ index]);
   ...
   if(index < 0) {
         index = mayinfo.length ();
         mayinfo.safe_push (out);
   }
...
perform_csipdfa_on_endblock (void)
{
   ...
   if (sp_block != main_firstbb) {
       if (pcsmap_required (cnode)) {
           ...
           ((cgraph_info)(cnode->symbol.aux))->pcsmap_required = false;
       }    
...
remove_from_df_list (unsigned int val, int *list)
{
     ...
     df_list bi  = (*list < 0) ? NULL : copy_df_list ( liveinfo[ *list]);
                                  ...
                                  int *gen, int *kill, int linfo)
{
   ...
   for (list=bi->stmt_constraint_chain; list; list=list->next) {
       constraint_t t = aliases[ list->val];
       struct constraint_expr lhs = t->lhs;
       struct constraint_expr rhs = t->rhs;
       bool lhs_live = false;
       int lhsvar = -1;
  ...
  // if(dump_file) && !ipacs_time) 
   ...
   if (cbb != main_firstbb) {
       if (lcsmap_required (cnode)) {
           ...
           ((cgraph_info)(cnode->symbol.aux))->lcsmap_required = false;
       }
       else {
           /*if (dump_file && !ipacs_time)
...
compute_live_parms (int index)
{
   ...
   for (bi=get_constraint_end (cbb); bi; bi=bi->prev) {
       if (bi->is_alias) {
           constraint_t t = aliases[ bi->stmt_constraint_chain->val];
...
struct cgraph_node *node;
        ...
        FOR_EACH_FUNCTION (node)
        {
                if (!gimple_has_body_p (node->symbol.decl) || node->clone_of)
                    continue;
                 push_cfun (DECL_STRUCT_FUNCTION (node->symbol.decl));
                 FOR_EACH_BB (bb)
                 {
